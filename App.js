...
export default function App() {
  ...
  const cards = [
    {
      ...
      caption: 'Clique em Tutoriais para acessar',
    }
  ];
  // END - CARDS
  ...
  changeIdade = (event, selectedDate) => {
    setShowIdade(false);
    ...
    <Block safe flex style={{ backgroundColor: '#F5F5F5' }}>
      ...
      {activeTab != "login" && <>
        {userData != null ? <Block row style={{ width: '100%', backgroundColor: '#3e0057', color: '#ffffff !important' }}>
          <Block flex>
            <Image source={require('./assets/header.png')} style={{ marginTop: 44, marginLeft: 20, width: "50%" }} />
          </Block>
          <Block flex right>
            <Text muted center color="#fff" style={{ marginTop: 54, width: "50%" }}>Olá, {userData.user.firstName}</Text>
          </Block>
        </Block> : null}
        <Block row style={{ width: '100%', paddingTop: 5, backgroundColor: '#3e0057', color: '#ffffff !important', paddingBottom: 10 }} />
        ...
        <Block style={styles.grid}>
          ...
          <ScrollView style={{ height: 1 }} keyboardShouldPersistTaps="handled">
            {activeTab == "home" && <>
              ...
              <Block row space="evenly">
                <Button color="" style={styles.button}>
                  <Block middle style={styles.block}>
                    <Icon size={40} color="#F5F5F5" name={'heart-pulse'} onPress={() => { setShowIdade(false); setIdade(new Date()); second("acol") }} />
                  </Block>
              ...
              <Block onClick={() => { tab("results"); second("results") }} onPress={() => { tab("results"); second("results") }}>
                {cards && cards.map((card, id) => (
                  <Card
                    onClick={() => { tab("results"); second("results") }}
                    onPress={() => { tab("results"); second("results") }}
                    key={`card-${card.image}`}
                    flex
                    borderless
            ...
            {activeTab == "results" && secondTab == "acol" && patientSelected != null && patientSelected == "new" && <>
              ...
              <Block style={styles.cardQuestion}>
                ...
                <Text muted center style={styles.buttonText}>Tempo de Estudo (anos)</Text>
                ...
                <Button round uppercase color="#3e0057" style={{ width: "100%" }} onPress={() => setShowIdade(true)}>Selecionar idade</Button>
                {showIdade && <DateTimePicker value={idade} mode={"date"} onChange={changeIdade} />}
                ...
                <Text muted center style={styles.buttonText}>Cor</Text>
                <TouchableOpacity style={styles.touchableOpacity}>
                  <Picker
                    mode="dialog"
                    style={styles.picker}
